...
if (empty($id_atencion)) { header("Location: atenciones.php"); exit; }

$res      = mysqli_query($con, "SELECT * FROM atenciones WHERE id_atencion = '$id_atencion'");
$atencion = mysqli_fetch_assoc($res);
if (!$atencion) { header("Location: atenciones.php"); exit; }

// Obtener datos del usuario actual
$res_usuario = mysqli_query($con, "SELECT CONCAT(rut, '-', dv) AS rut_completo, nombres, apellidos FROM usuarios WHERE id_usuario = '{$atencion['id_usuario']}'");
...
<div class="form-card">
    ...
    <h2 class="text-center">Editar Atención</h2>
    ...
    <form action="edit_atencion.php" method="POST">
        <input type="hidden" name="id_atencion" value="<?= $atencion['id_atencion'] ?>">
        <input type="hidden" name="id_usuario" id="id_usuario" value="<?= $atencion['id_usuario'] ?>">

        <!-- RUT -->
        <div class="mb-3">
            <label>RUT</label>
            <input type="text" id="rut_input" class="form-control" value="<?= $usuario['rut_completo'] ?>">
        </div>

        <!-- Nombre -->
        <div class="mb-3">
            <label>Nombre</label>
            <input type="text" id="nombre_usuario" class="form-control" readonly value="<?= $usuario['nombres'] ?>">
        </div>

        <!-- Apellido -->
        <div class="mb-3">
            <label>Apellido</label>
            <input type="text" id="apellido_usuario" class="form-control" readonly value="<?= $usuario['apellidos'] ?>">
        </div>
            ...
            <select name="id_causa" id="sel_causa" class="form-select" required>
                <?php foreach($causas as $c): ?>
                    <option value="<?= $c['id_causa'] ?>"
                        data-usuario="<?= $c['id_usuario'] ?>"
                        <?= $c['id_causa'] == $atencion['id_causa'] ? 'selected' : '' ?>>
                        <?= $c['rit'] ?>
                    </option>
                <?php endforeach; ?>
            </select>
...
// Filtrar causas al cargar la página
window.addEventListener('load', function() {
    const idUsuario   = String(document.getElementById('id_usuario').value);
    const causaActual = '<?= $atencion['id_causa'] ?>';
    const selCausa    = document.getElementById('sel_causa');
    selCausa.innerHTML = '';

    const filtradas = todasCausas.filter(opt => opt.dataset.usuario === idUsuario);
    ...
    fetch('../buscar_usuario.php?rut=' + rut)
        ...
        .then(data => {
            if (data.encontrado) {
                ...
                document.getElementById('sel_causa').innerHTML = '<option value="">Este usuario no tiene causas</option>';
            }
        });
});
</script>
...
async function initFlatpickr() {
    ...
    flatpickr("#fecha_atencion", {
        locale: "es",
        dateFormat: "Y-m-d",
        defaultDate: document.getElementById('fecha_atencion').value || "today",
        disable: [
